...
elif args.data in ['banana', 'breast_cancer', 'diabetis', 'flare_solar', 'german', 'heart', 'image', 'ringnorm', 'splice', 'titanic', 'waveform']:
	partitions = 10
...
for part in range(partitions):
	for cur_fold in range(kfold):
		if kfold > 1:
			...
			X, y, Xt, yt, split = read_data(key=args.data, return_split=True, preprocess=args.preprocess, inp_split=part)
		#print(X[0])
		#print(y[0:100])
		#print(Xt[0])
		#print(yt[0:100])
		...
		if(kfold > 1 or partitions > 1):
#			print("---------Partition/split done--------------")
	...
	if (kfold > 1):
		if args.file_write == False:
			print "\n\n\n\n"
			print "Dataset: " + iargs.data
			print "Number of levels: " + str(args.levels)
			#if( kfold > 1):
			print "Mean accuracy achieved: " + str(np.mean(all_acc))
			print "Var accuracy achieved: " + str(np.var(all_acc))
			#else:
			#	print "Maximum accuracy achieved: " + str(all_acc)
			print "Data dimensionality: " + str(dim)
			print "Number of training points: " + str(N)
			print "Number of test points: " + str(Xt.shape[0])
			...
			f = open("../results/" + str(args.data) + '_hmoe.txt' , 'a')
                        #if( kfold > 1):
			f.write(str(split) + ", " + str(args.levels) + ", " + str(args.reg_value) + ", " + str(np.mean(all_acc)) + ", "  + str(np.var(all_acc)) + "\n")
			#else:
			#f.write(str(split) + ", " + str(args.levels) + ", " + str(args.reg_value) + ", " + str(all_acc) + "\n")
